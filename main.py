    ...
    for qid, data in cursor_movements.items():
        ...
        if len(data.movements) > 1:
            ...
            stats["movement_details"][qid] = {
                ...
                "average_distance_per_movement": round(dist/count, 2)
            }

    stats["total_movements_all_questions"] = total_movements
    stats["average_movements_per_question"] = round(total_movements/len(cursor_movements), 2)
...
async def list_all():
    ...
    return {"count": len(data), "assessments": data}
